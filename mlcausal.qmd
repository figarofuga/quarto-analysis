...
treatment_effect <- ifelse(
  sex == 1,
  -0.30 + 0.90 * age_risk + 0.2 + 0.3 * bnp_risk + 0.2 * scale(bmi_risk) * scale(lvef_risk), 
  -0.50 + 0.80 * age_risk + 0.2 + 0.3 * bnp_risk + 0.4 * scale(bmi_risk) * scale(lvef_risk)
  )
...
toy_data <- dplyr::select(
  full_toy_data, 
  id, age, sex, bmi, hf, bnp, lvef, palpitation, ca, outcome, afeqt_os, eventtime, status
)
# ------------------------------------------------------------
# 10. Quick checks
# ------------------------------------------------------------
