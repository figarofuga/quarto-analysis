...
X = pd.get_dummies(
    lalonde_py[["age", "educ", "race", "married", "nodegree", "re74", "re75"]],
    columns=["race"],
    drop_first=True
)
p_score = ps_model.fit_predict(X, lalonde_py["treat"])
lalonde_py["propensity_score"] = p_score
...
# -----------------------
ps_model = LogisticRegressionPropensityModel()  # sklearn LogisticRegression 기반 :contentReference[oaicite:1]{index=1}
p_score = ps_model.fit_predict(lalonde_py[["age", "educ", "race", "married", "nodegree", "re74", "re75"]], lalonde_py["treat"])
...
# -----------------------
att = matched.loc[matched["treatment"] == 1, "outcome"].mean() - matched.loc[matched["treatment"] == 0, "outcome"].mean()
